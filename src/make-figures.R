...
setwd(savepath)
source('../src/CHANCE.R') # import "CHANCE.func" function
...
CHANCE.res <- CHANCE.func(Z, D, event, event.time) ###

alphas <- CHANCE.res$alphas
loglikelihoods <- CHANCE.res$loglikelihoods

plot(alphas, loglikelihoods)
plot(alphas, exp(loglikelihoods), 'l')

alpha.ML <- CHANCE.res$alpha.ML
alpha.min <- CHANCE.res$alpha.min
alpha.max <- CHANCE.res$alpha.max
    ...
    for (ii in 1:length(alpha.range)) {
      ...
      for (mm in 1 : num) {
        ...
        try ({
          ...
          alpha.ML.value[mm] <- CHANCE.res$alpha.ML
          alpha.ML.min[mm] <- CHANCE.res$alpha.min
          alpha.ML.max[mm] <- CHANCE.res$alpha.max
...
for (kk in 1:length(eta.range)) {
  for (jj in 1:length(alpha.range)) {
    ...
    df.plot$model <- factor(df.plot$model, levels = c('a', 'b', 'D', 'Z', 'CHANCE', 'c', 'd'))
    ...
    boxp.plot.func <- function(eta, alpha){
      ...
      legend("topright", c('D', 'Z', 'CHANCE'), col = c("cyan", 'blue', 'red'),
...
plot(N.range, run.time, ylim = c(0, 0.6), pch = 20,
     xlab = 'N', ylab = 'CHANCE run time (sec)')

write.csv(data.frame(N = N.range, run.time = run.time), 
          './Fig3-a-CHANCE-run-time-v1.csv', fileEncoding = 'GB2312')

pdf('./Fig3-a-CHANCE-run-time-v1.pdf', width = 5, height = 5)
plot(N.range, run.time, ylim = c(0, 0.6), pch = 20,
     xlab = 'N', ylab = 'CHANCE run time (sec)')
dev.off()
